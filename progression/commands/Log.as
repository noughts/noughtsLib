...
package jp.progression.commands {
	...
	/**
	 * <span lang="ja">Log クラスは、trace() メソッドを実行するコマンドクラスです。</span>
	 * <span lang="en"></span>
	 * 
	 * @example <listing version="3.0">
	 * // Log インスタンスを作成する
	 * var com:Log = new Log( "この文字列を出力します" );
	 * 
	 * // コマンドを実行する
	 * com.execute();
	 * 
	 * // Log インスタンスを作成する
	 * com = new Log( [ "配列の", "場合も", "正しく出力されます" ] );
	 * 
	 * // コマンドを実行する
	 * com.execute();
	 * 
	 * // Log インスタンスを作成する
	 * com = new Log( { aaa:"オブジェクトの場合", bbb:"中身が構造的に", ccc:"出力されます" } );
	 * 
	 * // コマンドを実行する
	 * com.execute();
	 * 
	 * // Log インスタンスを作成する
	 * com = new Log( function():String {
	...
	public class Log extends Command {
		...
		/**
		 * コピー元となる Log インスタンスです。
		 */
		private static var _target:Log;
		 ...
		 * <span lang="en"></span>
		 */
		public function Log( ... outputs:Array ) {
			// 引数を設定する
			if ( outputs.length > 1 ) {
				_output = outputs;
			}
		...
		/**
		 * <span lang="ja">Log インスタンスのコピーを作成して、各プロパティの値を元のプロパティの値と一致するように設定します。</span>
		 * <span lang="en">Duplicates an instance of an Log subclass.</span>
		 * 
		 * @return
		 * <span lang="ja">元のオブジェクトと同じプロパティ値を含む新しい Log インスタンスです。</span>
		 * <span lang="en">A new Log object that is identical to the original.</span>
		 */
